...
本文档详细介绍 LazyCat 客户端（iOS/Android）为前端应用注入的能力和 API。
...
先判断当前页面是否运行在客户端 WebShell 中，再决定是否调用宿主能力。
...
**推荐入口：** `AppCommon.LaunchApp(url, appid, options?)`
...
**推荐入口：** `AppCommon.ShareWithFiles(path?, paths?, mediaIds?)`
...
**推荐入口：** `AppCommon.ShareMedia({ actionName?, ids?, id? })`

**平台差异：**
- iOS：只需要传 `id` 或 `ids`
- Android：需要传 `actionName`，由包名和类名组成
...
**推荐入口：** `AppCommon.OpenWith(boxName, path, appid)`
...
**推荐入口：** `AppCommon.OpenAppTemporaryWindow(url)`
...
**推荐入口：** `SystemShare(contentOrPath)`
...
**推荐入口：** `AppCommon.OnVolumeKeyEvent(callback)`
...
**推荐入口：** `MediaSession` / `isMediaSessionAvailable()`
...
- `setPositionState` - 设置位置状态
- `setActionHandler` - 设置操作处理
  - `play`, `pause`, `nexttrack`, `previoustrack`
  - `seekto`, `stop`, `seekforward`, `seekbackward`

### 5.2 图片原生分享

**推荐入口：** `AppCommon.ShareImage(data)`
...
**建议组合：**
- 两端：`lzcapp-disable-dark` + `AppCommon.SetFullScreen`
- iOS：`lzcapp-navigation-bar-scheme=sink` + `lzcapp-state-bar-scheme=sink`
